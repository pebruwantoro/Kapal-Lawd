...
struct FindAuditagView: View {
    ...
    var body: some View {
        ...
        if audioPlayerViewModel.isFindBeacon && self.isFinding {
            @State var collections = audioPlayerViewModel.fetchCollectionByBeaconId(id: audioPlayerViewModel.beaconScanner.closestBeacon?.uuid.uuidString ?? "")
...
#Preview {
    FindAuditagView(isExploring: .constant(false), isFinding: .constant(false))
}
